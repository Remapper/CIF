...
#if defined(NDEBUG)
# error "CIF cannot be compiled without assertions."
#endif
...
int64_t GetBlockValue(int nBits, int nHeight, const CAmount& nFees)
{
	...
	if(nHeight > 1 && nHeight <= 200000) return 50 * COIN + nFees;
        if(nHeight > 200000 && nHeight <= 400000) return 25 * COIN + nFees;
        if(nHeight > 400000 && nHeight <= 600000) return 12.5 * COIN + nFees;
        if(nHeight > 600000 && nHeight <= 800000) return 6.25 * COIN + nFees;
        if(nHeight > 800000 && nHeight <= 1000000) return 3.125 * COIN + nFees;
        if(nHeight > 1000000) return 1.5 * COIN + nFees;
...
{
    int64_t ret = blockValue/2; // start at 50%
            ...
            else if (inv.IsKnownType())
            {
                ...
                if (!pushed && inv.type == MSG_DSTX) {       
                    if(mapPrivateSendBroadcastTxes.count(inv.hash)){
                        ...
                        ss.reserve(1000);
                        ss <<
                            mapPrivateSendBroadcastTxes[inv.hash].tx <<
                            mapPrivateSendBroadcastTxes[inv.hash].vin <<
                            mapPrivateSendBroadcastTxes[inv.hash].vchSig <<
                            mapPrivateSendBroadcastTxes[inv.hash].sigTime;
