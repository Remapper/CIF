...
// Copyright (c) 2014-2015 The CIF developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#if defined(HAVE_CONFIG_H)
#include "config/cif-config.h"
#endif
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += "  -conf=<file>           " + strprintf(_("Specify configuration file (default: %s)"), "cif.conf") + "\n";
...
#ifndef WIN32
    strUsage += "  -pid=<file>            " + strprintf(_("Specify pid file (default: %s)"), "cifd.pid") + "\n";
#endif
    ...
    strUsage += "  -port=<port>           " + strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 43199, 23799) + "\n";
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += "  -mintxfee=<amt>          " + strprintf(_("Fees (in CIF/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"), FormatMoney(CWallet::minTxFee.GetFeePerK())) + "\n";
    strUsage += "  -paytxfee=<amt>          " + strprintf(_("Fee (in CIF/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())) + "\n";
    ...
    strUsage += "                           cif (or specifically: privatesend, instantx, masternode, keepass, mnpayments, mnbudget)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        strUsage += ", qt";
    strUsage += ".\n";
#ifdef ENABLE_WALLET
    ...
    strUsage += "  -minrelaytxfee=<amt>   " + strprintf(_("Fees (in CIF/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())) + "\n";
    ...
    strUsage += "  -litemode=<n>          " + strprintf(_("Disable all CIF specific functionality (Masternodes, PrivateSend, InstantX, Budgeting) (0-1, default: %u)"), 0) + "\n";
    ...
    strUsage += "  -masternodeaddr=<n>        " + strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:43199") + "\n";
    ...
    strUsage += "\n" + _("PrivateSend options:") + "\n";
    strUsage += "  -enableprivatesend=<n>          " + strprintf(_("Enable use of automated privatesend for funds stored in this wallet (0-1, default: %u)"), 0) + "\n";
    strUsage += "  -privatesendrounds=<n>          " + strprintf(_("Use N separate masternodes to anonymize funds  (2-8, default: %u)"), 2) + "\n";
    strUsage += "  -anonymizecifamount=<n>     " + strprintf(_("Keep N CIF anonymized (default: %u)"), 0) + "\n";
    strUsage += "  -liquidityprovider=<n>       " + strprintf(_("Provide liquidity to PrivateSend by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0) + "\n";

    strUsage += "\n" + _("InstantX options:") + "\n";
    strUsage += "  -enableinstantx=<n>    " + strprintf(_("Enable instantx, show confirmations for locked transactions (bool, default: %s)"), "true") + "\n";
    strUsage += "  -instantxdepth=<n>     " + strprintf(_("Show N confirmations for a successfully locked transaction (0-43199, default: %u)"), nInstantXDepth) + "\n";
    ...
    strUsage += "  -rpcport=<port>        " + strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 43198, 23798) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2014-%i The CIF Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that CIF is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
...
#endif
    // Make sure only a single CIF process is using the data directory.
    ...
    if(nLiquidityProvider != 0) {
        ...
        nPrivateSendRounds = 431999;
    }

    nAnonymizeDarkcoinAmount = GetArg("-anonymizecifamount", 0);
    if(nAnonymizeDarkcoinAmount > 4319999) nAnonymizeDarkcoinAmount = 4319999;
    ...
    LogPrintf("Anonymize CIF Amount %d\n", nAnonymizeDarkcoinAmount);
