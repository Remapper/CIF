...
/* *** BEGIN PrivateSend MAGIC - CIF **********
    Copyright (c) 2014-2015, CIF Developers
        eduffield - evan@cifpay.io
        udjinm6   - udjinm6@cifpay.io
*/
    ...
    switch(state) {
        ...
        case POOL_STATUS_ACCEPTING_ENTRIES:
            if(entriesCount == 0) {
                ...
                std::string suffix = "";
                if(     showingPrivateSendMessage % 70 <= 40) return strprintf(_("Submitted following entries to masternode: %u / %d"), entriesCount, GetMaxPoolTransactions());
                else if(showingPrivateSendMessage % 70 <= 50) suffix = ".";
                else if(showingPrivateSendMessage % 70 <= 60) suffix = "..";
                else if(showingPrivateSendMessage % 70 <= 70) suffix = "...";
        ...
        case POOL_STATUS_SIGNING:
            if(     showingPrivateSendMessage % 70 <= 40) return _("Found enough users, signing ...");
            else if(showingPrivateSendMessage % 70 <= 50) suffix = ".";
            else if(showingPrivateSendMessage % 70 <= 60) suffix = "..";
            else if(showingPrivateSendMessage % 70 <= 70) suffix = "...";
        ...
        case POOL_STATUS_QUEUE:
            if(     showingPrivateSendMessage % 70 <= 30) suffix = ".";
            else if(showingPrivateSendMessage % 70 <= 50) suffix = "..";
            else if(showingPrivateSendMessage % 70 <= 70) suffix = "...";
...
bool CPrivateSendPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    if (!pwalletMain->SelectCoinsDark(nValueMin, nBalanceNeedsAnonymized, vCoins, nValueIn, 0, nPrivateSendRounds))
    {
        ...
        if (pwalletMain->SelectCoinsDark(nValueMin, 43199999*COIN, vCoins, nValueIn, -2, 0))
        {
...
bool CPrivateSendPool::CreateDenominated(int64_t nTotalValue)
{
    ...
    // ****** Add denoms ************ /
    BOOST_REVERSE_FOREACH(int64_t v, PrivateSendDenominations){
        int nOutputs = 0;
...
bool CPrivateSendSigner::VerifyMessage(CPubKey pubkey, vector<unsigned char>& vchSig, std::string strMessage, std::string& errorMessage)
{
    ...
    if (fDebug && pubkey2.GetID() != pubkey.GetID())
        LogPrintf("CPrivateSendSigner::VerifyMessage -- keys don't match: %s %s\n", pubkey2.GetID().ToString(), pubkey.GetID().ToString());
...
bool CPrivateSendQueue::CheckSignature()
{
    ...
    if(pmn != NULL)
    {
        ...
        std::string errorMessage = "";
        if(!PrivateSendSigner.VerifyMessage(pmn->pubkey2, vchSig, strMessage, errorMessage)){
