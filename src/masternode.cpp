...
void CMasternode::Check(bool forceCheck)
{
    ...
    if(!unitTest){
        ...
        CTxOut vout = CTxOut(999.99*COIN, PrivateSendPool.collateralPubKey);
...
bool CMasternodeBroadcast::CheckAndUpdate(int& nDos)
{
    ...
    if(!PrivateSendSigner.VerifyMessage(pubkey, sig, strMessage, errorMessage)){
    ...
    if(Params().NetworkID() == CBaseChainParams::MAIN) {
        if(addr.GetPort() != 43199) return false;
    } else if(addr.GetPort() == 43199) return false;
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    // should be at least not earlier than block when 1000 CIF tx got MASTERNODE_MIN_CONFIRMATIONS
    uint256 hashBlock = 0;
    CTransaction tx2;
