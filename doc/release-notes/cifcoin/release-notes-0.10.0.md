...
- saction for PrivateSend. These are special transactions that require a signature that only the masternodes can create.
- PrivateSend now has no fees to track what-so-ever, all that will ever be in PrivateSend transactions are PrivateSend denominations.
...
- Split up privatesend.cpp into masternode.cpp, activemasternode.cpp and instantx.cpp
- Added modular ProcessMessages for PrivateSend, Masternodes and InstantX
...
- Fixed all sorts of issues with collateral and PrivateSend.
...
- PrivateSend tested with 2.5 DRK successfully and I've set that as the minimum
...
- PrivateSend UI is disabled for masternodes now and titlebar says "[masternode]"
...
- Updated wording for privatesend messages
- Removed disable privatesend checkbox from config screen
- Added "-enableprivatesend" cli option
...
- PrivateSend should anonymize very close to the intended amount now
- Added Amount/Rounds to overview screen so you can see current settings
- Overview privatesend cache is cleared on settings change (will instantly update)
...
- This update deals with freezing, slow wallets, slow load times and the
  ...
  PrivateSend process has been reserve keys for every attempt! To rectify this
...
- Easy to read privatesend progress bar (Mouse over for very detailed info)
...
- Client now creates fee sizes inputs for use in privatesend denomination phases
...
- Client now prioritises sending denominated funds back into PrivateSend. This will
...
- PrivateSend now checks diskspace before sending anything to a masternode.
...
- Majorly improved the way privatesend participants are paired together. It should
  be super fast now.
...
- PrivateSend now shows address instead of "n/a"
...
- Added GUI configuration for PrivateSend Rounds, Enable PrivateSend and Anonymize
  amount of DRK
...
- PrivateSend limited to 5000DRK per wallet.dat. Client will warn about this the
  first time it's opened, then disable privatesend from then on.
...
- Found race condition with new blocks and clearing privatesend entries that was
...
- PrivateSend Denominate Outputs are now in a random order:
...
- PrivateSend now uses 3 participants instead of two.
- SplitUpMoney can now make collateral inputs when needed
- Transactions now shows privatesend transaction types for easier understanding
  of what's going on:
...
- Fixed the money destruction bug, it was caused by "privatesend denominate 8000".
  I missed a reference and the client passed an empty address to
  SendMoneyToDestination. rcp privatesend source: http://pastebin.com/r14piKuq
...
- Added PrivateSend detection to the UI
- fixed senttoaddress, it will use all inputs when privatesend is disabled now.
  Otherwise it will ONLY use non-denom.
- "privatesend addr amount" now returns the hash of the transaction it creates
...
- Removed "privatesend denominate", privatesend now will figure out the most it can
  denominate. Use "privatesend auto" instead.
...
- QT GUI will now ask to unlock the wallet when it detects PrivateSend wants to do
  something and lock it when it's done again.
- PrivateSend is turned off by default in the daemon now. In most cases daemons
  won't want to run with anonymity (pools, exchanges, etc), if a user does they
  can override the default setting with -enabledaemonprivatesend=true
- Fees per round of PrivateSend are 0.001DRK or $0.00538 at current prices. This
...
- RPC calls are changed a bit:
    ...
    privatesend denominate 100 - Will denominate 100DRK
    privatesend auto - Will run AutoDenominate
    privatesend Xaddr 100 - Will send 100 denominated DRK to Xaddr
...
- Fixed privatesend+ rounds function, it was returning bad data in some cases
  causing privatesend inputs to never stop being sent through privatesend.
...
/darkcoin-qt -datadir=/home/user/.darkcoin -listen=0 -privatesendrounds=8 -usemasternode="192.168.56.102:23799"
/darkcoin-qt -datadir=/home/user/.darkcoin2 -listen=0 -privatesendrounds=8 -usemasternode="192.168.56.102:23799"

and even disable privatesend auto-denom if wanted:
/darkcoin-qt -datadir=/home/user/.darkcoin -listen=0 -privatesendrounds=8 -usemasternode="192.168.56.102:23799" -disableprivatesend=1
...
- Added a smart input splitting method. Place 1000+DRK into a brand new wallet
  and it will be split into many inputs compatible with PrivateSend
...
- PrivateSendRounds had a bug that I fixed, should calculate correctly now
...
- New column "PrivateSend Rounds" in coincontrol to show how secure a given input is
- Fixed a few issues causing privatesend to fail. We should see many more privatesends
...
- Redid denominations to 1, 10, 100, and 500. Maybe this is too simple, but it
  ...
  again through privatesend for the next transactions. We'll see how it works.
...
- Improved PrivateSendInput add entry verification. Masternodes will now reject
