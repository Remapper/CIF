...
- Reduced lower privatesend limit to 1.5 DRK
...
- Added more detailed PrivateSend status information to Overview tab
- Added PrivateSend high precision matching engine
- Added PrivateSend balance to `getinfo`
...
- Add '-disablewallet' mode to let cifd run entirely without wallet (when
  built with wallet)
- Update default '-rpcsslciphers' to include TLSv1.2
- make '-logtimestamps' default on and rework help-message
- RPC client option: '-rpcwait', to wait for server start
- Remove '-logtodebugger'
- Allow `-noserver` with cifd
